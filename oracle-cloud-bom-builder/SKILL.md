...
12. For BOMs that depend on calculator exports or eSource rows, run `scripts/check_pricing_refresh.py` before finalizing. Provide `--current-esource-date` from the authenticated eSource PDF when eSource rows are present. Add `--minimum-esource-date "July 1, 2026"` for eSource-priced Exadata X11 storage-server, Database Server-Z, XT, and RA26/RA26-Z-related rows introduced by the June 30, 2026 announcement.
...
- Treat the current authenticated Oracle eSource price-list PDF as an authoritative internal price-list source when it is available, date-checked, and the needed row can be found exactly outside calculator coverage.
