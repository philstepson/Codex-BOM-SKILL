...
## Invocation Pricing Reminder

When a request may require pricing from the Oracle eSource price list, tell the user before pricing that the repository is using the eSource PDF date recorded in `tmp/esource-price-list/oracle-paas-iaas-public-cloud-global-price-list.metadata.json`. Read that metadata and state its `document_date`. Ask the user to compare it with the current PDF date at `https://esource.oraclecorp.com/sites/eSource/ContentAsset_1530207473152` and to download the current PDF if the eSource page shows a newer date. Do not treat the repository cache as current until that date check is satisfied or the user explicitly accepts using the cached date as an unverified source.
...
1. Gather inputs from the user or source file:
   ...
   - Supplemental pricing source details when calculator data is incomplete, especially the current Oracle eSource PDF URL and a locally downloaded current PDF from the user's authenticated Oracle session. For Exadata X11 storage-server and RA26/RA26-Z-related rows announced June 30, 2026, use current Oracle pricing calculator/export rows when available, or the July 1, 2026 eSource price list or newer when calculator coverage is missing.
2. Decide pricing source before extracting rows. Treat current Oracle customer-facing pricing calculator, Cost Estimator, or calculator export rows as authoritative for calculator-covered resources, including newly exposed Exadata X11 SKU rows, because they provide SKU rows, quantities, unit prices, and monthly costs together. Treat the current authenticated Oracle eSource price-list PDF as an authoritative internal price-list source when it is available, date-checked, and exact rows are needed outside calculator coverage. Transpose calculator rows into the BOM format without changing the calculator price fields.
...
11. Use eSource only after the calculator path is unavailable or incomplete, when the request is Exadata Cloud@Customer and the calculator does not expose the needed infrastructure rows, or when the user gives a SKU that must be found in the price-list PDF. Ask the user to authenticate to eSource and download the current PDF when needed, then verify the managed Oracle eSource price-list PDF cache before extracting supplemental prices. Do not rely on Chrome extension access to the PDF viewer as the normal eSource workflow. Read `references/esource-price-list-cache.md` before using cached PDF pricing.
12. For BOMs that depend on calculator exports or eSource rows, run `scripts/check_pricing_refresh.py` before finalizing. Provide `--downloaded-esource-pdf <path>` for the current PDF downloaded from the authenticated eSource page when eSource rows are present, or use `--current-esource-date` only as a manual fallback. Add `--minimum-esource-date "July 1, 2026"` for eSource-priced Exadata X11 storage-server, Database Server-Z, XT, and RA26/RA26-Z-related rows introduced by the June 30, 2026 announcement.
...
Prioritize Oracle Platform Services. Also include any OCI service the user names explicitly. Examples include Exadata Database Service, Autonomous Database, Base Database Service, Zero Data Loss Recovery Appliance, Compute, Block Volume, Object Storage, Load Balancer, VCN, DRG, FastConnect, OKE, Logging, Monitoring, WAF, IAM, and Security Zones.
...
When the requested BOM includes on-premises Exadata Database Machine X11M, Exadata Storage Expansion Rack X11M, Exadata X11 storage servers, Database Server-Z, XT storage, or Zero Data Loss Recovery Appliance RA26/RA26-Z, read `references/exadata-database-machine-x11m.md` before asking questions or preparing rows. Use it to validate X11 versus X11M storage behavior, XRMEM assumptions, HC/EF/HC-Z/XT limits, storage expansion rules, RA26/RA26-Z capacity facts, and known July 1 price-list SKU rows. If the pricing calculator exposes these SKUs, use the calculator/export rows as the working pricing source.

For Exadata Cloud@Customer, explicitly gather model-level resource details because Oracle Cost Estimator may not cover the C@C infrastructure pricing path. Ask for database server model/count, storage server model/count, storage server generation (`X11M` with XRMEM or `X11` without XRMEM where available), rack configuration, and any other Cloud@Customer infrastructure components the user expects in the BOM.
...
- Treat current Oracle customer-facing pricing calculator, Cost Estimator, or calculator export rows as authoritative for calculator-covered resources, including newly exposed Exadata X11 SKUs, because they provide SKU rows, quantities, unit prices, and monthly costs together.
- Treat the current authenticated Oracle eSource price-list PDF as an authoritative internal price-list source when it has been downloaded from the authenticated session, date-checked locally, and the needed row can be found exactly outside calculator coverage.
- If a current customer-facing Oracle calculator/site exposes a SKU or price that is not yet reflected in the latest eSource PDF, use the calculator/site output as the working BOM source and preserve the source name/date in `Custom Note`. Do not backfill or override that row from an older eSource cache.
...
- If Oracle Cost Estimator or the pricing calculator cannot provide the needed SKU or price for a row, use the current authenticated Oracle eSource PDF as the authoritative fallback source after the user downloads it and the script checks its document date.
- For Exadata X11 storage-server, Database Server-Z, XT, and Zero Data Loss Recovery Appliance RA26/RA26-Z-related rows introduced by the June 30, 2026 Oracle announcement, use current Oracle pricing calculator/export rows when available. If pricing from eSource instead, require the July 1, 2026 price list or newer and do not price these rows from the June 11, 2026 cache. The July 1, 2026 PaaS/IaaS price list in this repository did not expose RA26/RA26-Z hardware SKU rows, so ask for a current Recovery Appliance price-list row, calculator/export row, or formal quote source before pricing RA26 hardware.
...
- The current supplemental PDF URL is `https://esource.oraclecorp.com/sites/eSource/ContentAsset_1530207473152`; open it through browser authentication when needed, then download the PDF for local validation.
- A local PDF cache is allowed for repeatability, but it must be refreshed from a newly downloaded eSource PDF before each pricing run when the downloaded document date is newer than the cached document date. Follow `references/esource-price-list-cache.md`.
- Run `scripts/check_pricing_refresh.py` as a preflight for calculator/eSource source checks. The script verifies cache metadata, extracts the date from a supplied downloaded eSource PDF, can refresh the managed cache from that download, can enforce a required minimum eSource date, scans CSV source notes, and warns when standard Dedicated/Database@ Exadata rows appear to rely on eSource instead of calculator exports.
...
`scripts/check_pricing_refresh.py` checks eSource cache metadata, the cached PDF, document date extracted from a downloaded authenticated eSource PDF, source notes in CSV input rows, and recent calculator exports in `~/Downloads`.
