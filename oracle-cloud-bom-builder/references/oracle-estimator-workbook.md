...
For calculator-covered resources, Oracle Cost Estimator, Oracle pricing calculator, or current customer-facing Oracle pricing site output is authoritative and is usually the primary workflow source because it provides SKU rows, quantities, unit prices, and monthly costs together. This includes newly exposed Exadata X11 SKUs when the current calculator shows them. Use the current authenticated Oracle eSource price-list PDF as an authoritative internal price-list source when it is available, date-checked, and exact rows are needed outside calculator coverage. Transpose calculator rows into the BOM and add discount/formula columns without changing the calculator price fields.
...
When an estimator row lacks needed SKU or price data, especially for Exadata Cloud@Customer database servers, storage servers, rack components, or related infrastructure, check the current Oracle pricing calculator/export first. If the customer-facing calculator/site exposes a SKU that eSource has not caught up to yet, use the calculator/site output as the working BOM source and note that source in the row. Use the current authenticated Oracle eSource PDF after document-date validation when the calculator/export path is unavailable, incomplete, suspect, or when the request requires price-list-only rows.
