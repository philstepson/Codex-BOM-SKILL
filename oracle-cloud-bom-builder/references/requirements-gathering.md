...
- If the SKU is one-time and should be included in the annual estimate, leave recurring monthly cost blank, calculate one-time list cost once from quantity and unit price, and include it in total annual cost and discounted total annual cost.

Before asking the user to authenticate to eSource, determine whether Oracle Cost Estimator or the Oracle pricing calculator can cover the requested rows because the calculator is usually the fastest authoritative workflow source. When eSource is available and date-checked, treat it as the definitive price list. For Exadata Cloud@Customer and other resources that Oracle Cost Estimator or the Oracle pricing calculator does not fully cover, use the user's current authenticated Oracle eSource PDF. A local PDF cache may be used only after checking the current eSource document date. Replace the cached PDF if eSource has a newer document date, then extract only the relevant runtime rows, capture the document date from the PDF front page, and add a BOM note for any row priced from that PDF. Do not treat previously extracted pricing tables as authoritative across runs.
